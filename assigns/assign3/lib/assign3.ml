...
let rec ntree_of_tree (t : 'a tree) : 'a Stdlib320.ntree option =
  ...
  | Node2 (x, left, right) ->
      ...
      in
      Some (Stdlib320.Ntree.create x children)
...
let file_tree root paths =
    ...
    | part :: rest ->
        let child =
          ...
          | Some (Stdlib320.Ntree.Node (_, subchildren)) ->
              Stdlib320.Ntree.create part (insert_path rest subchildren)
          | None ->
              Stdlib320.Ntree.create part (insert_path rest [])
        in
  ...
  in
  Stdlib320.Ntree.create root root_children
